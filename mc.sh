...
# Env
PROJECT="paper"
#LATEST_VERSION=$(wget -O -s https://api.papermc.io/v2/projects/${PROJECT} | jq -r '.versions[-1]')
MC_VERSION="1.21.4"
LATEST_BUILD=$(wget -O -s https://api.papermc.io/v2/projects/${PROJECT}/versions/${MC_VERSION}/builds | jq -r '.builds | map(select(.channel == "default") | .build) | .[-1]')
